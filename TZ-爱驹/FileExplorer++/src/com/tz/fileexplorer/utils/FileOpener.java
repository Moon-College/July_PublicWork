...
public class FileOpener {
	...
	public static void openImage(Context c, File file, String[] mSuffix) {
		Intent intent = createVariousIntent(file, 0, getImageIntent(file),
				mSuffix);
	...
	public static void openMusic(Context c, File file, String[] mSuffix) {
		Intent intent = createVariousIntent(file, 1, getMusicIntent(file),
				mSuffix);
	...
	public static void openTextFile(Context c, File file, String[] mSuffix) {
		Intent intent = createVariousIntent(file, 2, getTextIntent(file),
				mSuffix);
	...
	private static Intent createVariousIntent(File file, int index,
		...
		// --core code--
		for (int i = 0; i < suffix[index].length; i++) {
			if (fileName.endsWith(suffix[index][i])) {
				return intent;
			} else {
	...
	private static Intent openOtherFiles(File file) {
		...
		for (int i = 0; i < suffix.length; i++) {
			for (int j = 0; j < suffix[i].length; j++) {
				if (!fileName.endsWith(suffix[i][j])) {
					// intent = new Intent();
