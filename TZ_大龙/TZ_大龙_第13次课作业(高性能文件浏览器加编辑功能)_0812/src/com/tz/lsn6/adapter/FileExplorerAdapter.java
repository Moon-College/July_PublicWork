...
//		mlayoutInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null) {
			...
			holder.holderDetail = (TextView) convertView.findViewById(R.id.bt_detail);
		 ...
		 * 添加监听
		 */
		holder.holderDetail.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if (mCallback != null) {
					mCallback.onClick(v, location, R.id.tv_attr);
				}
			}
		});
	...
	class LoaderIconAsyncTask extends AsyncTask<String, integer, Void> {
		...
		protected Void doInBackground(String... params) {
			...
			if(bitmap == null) {
				//将图片进行压缩 再存入缓存中
				bm = ImageLoader.decodeBitmapFromResoure(path, Math.round(windowWidth/ICON_WIDTH_SCALE));
